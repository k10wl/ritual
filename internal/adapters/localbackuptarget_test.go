...
func TestNewLocalBackupTarget(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			target, err := NewLocalBackupTarget(tt.storage, context.Background())
...
func TestLocalBackupTarget_Backup(t *testing.T) {
	// Setup
	mockStorage := &mocks.MockStorageRepository{}
	target, err := NewLocalBackupTarget(mockStorage, context.Background())
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func() {
				mockStorage.ListFunc = func(ctx context.Context, prefix string) ([]string, error) {
					return []string{}, nil // No existing backups
				}
		...
		{
			name:          "storage list error during skip check",
			data:          []byte("test backup data"),
			wantError:     true,
			errorContains: "failed to check backup frequency",
			setupMocks: func() {
				mockStorage.ListFunc = func(ctx context.Context, prefix string) ([]string, error) {
...
func TestLocalBackupTarget_DataRetention(t *testing.T) {
	// Setup
	mockStorage := &mocks.MockStorageRepository{}
	target, err := NewLocalBackupTarget(mockStorage, context.Background())
	require.NoError(t, err)
	...
	t.Run("no retention needed when under limit", func(t *testing.T) {
		// Create 5 backup files (under limit of 10)
		mockFiles := []string{}
		for i := 0; i < 5; i++ {
	...
	t.Run("removes excess backups", func(t *testing.T) {
		// Create 15 backup files (exceeds limit of 10)
		mockFiles := []string{}
		for i := 0; i < 15; i++ {
			timestamp := time.Now().Add(-time.Duration(i) * time.Hour).Format("20060102150405")
			mockFiles = append(mockFiles, fmt.Sprintf(config.LocalBackups+"/%s.zip", timestamp))
		...
		err := target.DataRetention()
		assert.NoError(t, err)
		assert.Equal(t, 5, len(deletedFiles), "Expected 5 files to be deleted (15 - 10 limit)")
	})
...
func TestLocalBackupTarget_BackupSkipping(t *testing.T) {
	mockStorage := &mocks.MockStorageRepository{}
	target, err := NewLocalBackupTarget(mockStorage, context.Background())
	require.NoError(t, err)

	t.Run("skips backup when newest file is from same month", func(t *testing.T) {
		// Create a backup file from current month
	...
	t.Run("creates backup when newest file is from different month", func(t *testing.T) {
		// Create a backup file from previous month
		lastMonth := time.Now().AddDate(0, -1, 0)
		lastMonthTimestamp := lastMonth.Format("20060102150405")
		mockFiles := []string{
			fmt.Sprintf(config.LocalBackups+"/%s.zip", lastMonthTimestamp),
		}
	...
	t.Run("handles multiple files with newest from same month", func(t *testing.T) {
		// Create multiple backup files with newest from current month
		currentTime := time.Now()
		currentMonthTimestamp := currentTime.Format("20060102150405")
		...
		mockFiles := []string{
			fmt.Sprintf(config.LocalBackups+"/%s.zip", currentMonthTimestamp), // newest
			fmt.Sprintf(config.LocalBackups+"/%s.zip", lastMonthTimestamp),    // older
		}
...
func TestLocalBackupTarget_InterfaceCompliance(t *testing.T) {
	mockStorage := &mocks.MockStorageRepository{}
	target, err := NewLocalBackupTarget(mockStorage, context.Background())
	require.NoError(t, err)
...
func TestLocalBackupTarget_NilReceiver(t *testing.T) {
