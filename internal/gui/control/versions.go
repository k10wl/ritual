...
// metadata. Unparseable keys are skipped. The newest id is flagged IsHead. A
// ref whose body can't be read is included with zero counts rather than
// failing the whole listing — a half-written ref shouldn't hide the rest.
func listScope(ctx context.Context, src VersionScope, source string) ([]Version, error) {
	...
	for _, id := range ids {
		...
		v := Version{ID: string(id), IsHead: id == head, Source: source}
