...
type fakeLister struct {
	ifs []netinfo.Interface
	err error
}

func (f fakeLister) List() ([]netinfo.Interface, error) {
	return f.ifs, f.err
}
...
func TestAddressProvider_FiltersInterfacesAndFormatsPort(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			provider := netinfo.NewAddressProvider(25565, fakeLister{ifs: tc.ifs})
...
func TestAddressProvider_ListerError_DegradesToLocalhostOnly(t *testing.T) {
	provider := netinfo.NewAddressProvider(25565, fakeLister{err: listerError{}})
	got := provider.Addresses()
