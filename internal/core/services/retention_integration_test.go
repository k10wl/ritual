...
func TestRetention_IntegrationFS_MixedFormats(t *testing.T) {
	...
	for _, ts := range []string{"20260414160000", "20260413160000", "20260412160000"} {
		...
		if err := os.WriteFile(filepath.Join(d, "level.dat"), []byte("x"), 0o644); err != nil {
			t.Fatal(err)
		}
	}

	// v1 tar backups
	if err := os.WriteFile(filepath.Join(backups, "20260411160000.tar"), []byte("old"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(backups, "20260410160000.tar"), []byte("older"), 0o644); err != nil {
		t.Fatal(err)
	}

	// Unknown file (should be deleted by sacred-dir rule)
	if err := os.WriteFile(filepath.Join(backups, "garbage.txt"), []byte("noise"), 0o644); err != nil {
		t.Fatal(err)
	}
