...
3. **Show type signatures**: Go core (`cmd/`, `internal/`) + TypeScript frontend (`frontend/src/`, Lit + Wails bindings). Use Go signatures for backend, TS for frontend; don't assume one stack.
...
Caller-facing tool description prose follows design-log/032 §Rule (R1''' headline + optional value paragraph + trigger phrases; R2' arg micro-template; R3 wire detail confined to design log + handler code).

## Design Log Index

1. **Maintain `./design-log/index.md`**: Single chronological table of every design log. No category sections, no parallel lists.
2. **Before reading**: Check index.md first to find relevant design logs.
3. **After creating/updating**: Append the new entry to the bottom of the table.
4. **Table format**:
```markdown
   | #   | Title | Date | Status | Description |
   |-----|-------|------|--------|-------------|
   | NNN | title | YYYY-MM-DD | Draft/Approved/Implemented/Superseded | brief description |
```