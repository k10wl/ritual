...
| 1 | Progress bar reads 0% — push/pull never emit a `PlanInfo{BytesTotal}` event upfront | **closed on `feat/delta-sync` `47249ed`** — `ritual.PlanInfo{Operation, BytesTotal, FilesTotal}` event added; `refs.Puller`/`refs.Pusher` gain optional `OnPlan(PlanFn)` setter (no constructor break across 33 callsites), composition root wires both to `bus.Publish`. Projection folds the event into `state.BytesTotal`/`state.FilesTotal`. Five regression tests across refs, projection, and integration lock the contract |
| 2 | Pre-existing integration timeouts: `TestIntegration_ChangesUploaded_RefAppearsOnRemote`, `TestIntegration_ServerLifecycleEventsEmitted_StartingReadyStoppingStopped` (1s `waitDone` budget) | **not reproducible on `feat/delta-sync`** — both tests pass consistently. Stress run 100× passed in 7.6s aggregate (~76ms/test) and 20× under `-race` passed in 3.7s. The 1s budget is met with >10× headroom. Likely fixed incidentally by the state-machine refactor + bus-driven Stop pattern that landed earlier on this branch; no targeted fix needed |
| 3 | Stop only graceful in Running stage; Pulling/Pushing aborts wait the 20s window-close budget | **closed on `feat/delta-sync` `4ce5ebd`** — Pulling and Pushing subscribe to `ritual.StopRequested` and cancel a local stop-ctx so Pull/Push unblock fast. Pulling routes through onFail (no lock acquired yet). Pushing introduces an optional `OnStop` edge wired by pipeline.Build to unlocking so the held lock is released on user-initiated abort; falls back to onFail when unset. Three regression tests lock the contract |
| 4 | `cmd/gui/main.go` mock-remote TODO — needs `adapters.NewR2Repository` swap | **closed on `feat/delta-sync` `855f8fb`** — `internal/subsystems/remote/Build` factory branches on `settings.RemoteR2`: nil → throttled local-FS mock (alpha default), populated → `adapters.NewR2Repository`. Operators flip backends by editing `settings.json` — no code change, no rebuild. Cmd/gui's inline mock plumbing replaced with one factory call. Four regression tests lock the contract (default-nil, save/load round-trip, mock-on-disk path, R2-error-wrapping) |
